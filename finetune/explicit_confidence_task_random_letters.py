...
def analyze_results(results, model_name="Model", mode_str=""):
    ...
    # ECE and Brier
    try:
        from evaluation_metrics import compute_expected_calibration_error, compute_brier_score
...
def main():
    ...
    if args.test_type != "baseline":
        ...
        base_model, tokenizer, _ = load_model_and_tokenizer(args.base_model)
    ...
    if args.lora_repo and args.test_type != "baseline":
        ...
        ft_model, tokenizer, _ = load_model_and_tokenizer(
            base_model_name=args.base_model, adapter_path=args.lora_repo,
        )
        ft_model.to(device)
        ...
        if args.test_type == "baseline":
            print("Loading base model...")
            model, tokenizer, _ = load_model_and_tokenizer(args.base_model)
            model.to(device)
