...
def _atomic_savez_compressed(path: str, **arrays) -> None:
    ...
    Subtlety: np.savez_compressed APPENDS '.npz' when the destination path
    doesn't already end in '.npz' — so we can't naively use '<final>.tmp' as
    the staging path (numpy would write '<final>.tmp.npz' and the rename
    would point at the wrong file). Instead, the staging path itself ends
    in '.npz' so numpy writes it verbatim, and the suffix '.tmp.npz' makes
    the staging-vs-final distinction explicit.
    """
