...
#     """Build MCQ prompts - manually skip system message entirely."""
...
#         user_content = setup_prompt + "\n\n" + q_text + "Your choice (A, B, C, or D):"
        
#         # Manually build - NO system message at all
#         prompt = (
#             "<|begin_of_text|>"
...
#     """
#     MCQ pass using generation + parsing 
#     """
#     enc = tokenizer(prompts, return_tensors="pt", padding=True).to(device)

#     # Still get logits for entropy
#     if requires_grad:
...
#     # Get aggregated logits for entropy (unchanged)
#     letter_token_ids = {
...
#     # BUT: Use generation for actual answer (more robust)
#     with torch.no_grad():
#         gen = model.generate(
#             **enc,
#             max_new_tokens=10,  # Allow more tokens in case model explains
#             do_sample=False,
#         )
    
#     # Decode and parse
#     decoded = tokenizer.batch_decode(gen[:, enc["input_ids"].shape[1]:])
    
#     pred_letters = []
#     for text in decoded:
#         letter = parse_letter_from_model_text(text, "ABCD")
#         pred_letters.append(letter if letter else "?")

#     # Debug: print first 5 responses
#     # debug_print = True
#     # if debug_print:  # Set True for one-time debug
#     #     print("\nDEBUG: Model generated text (first 5):")
#     #     for i, text in enumerate(decoded[:5]):
#     #         print(f"  {i}: {repr(text)} -> parsed as '{pred_letters[i]}'")
...
#     MCQ pass:
#       • One forward pass
#       • Extract A–D logits (aggregating ' A'/A variants)
...
#         requires_grad: If True, keeps gradients (for training). If False, uses no_grad (for evaluation)
#     """

#     enc = tokenizer(prompts, return_tensors="pt", padding=True).to(device)

#     # Conditionally use no_grad based on whether we need gradients
#     if requires_grad:
#         out = model(**enc, use_cache=False)
#     else:
#         with torch.no_grad():
...
#     # Get all token IDs for each letter (handles both 'A' and ' A')
#     letter_token_ids = {
#         letter: get_letter_token_ids(tokenizer, letter) 
#         for letter in "ABCD"
#     }
    
#     # Aggregate logits for each letter across all its token variants
#     # Use logsumexp for numerical stability
#     logits4_list = []
#     for letter in "ABCD":
#         token_ids = letter_token_ids[letter]
#         # Get logits for all variants of this letter
#         letter_logits = final_logits[:, token_ids]  # [B, num_variants]
        
#         # Aggregate using logsumexp (equivalent to summing probabilities)
#         # log(p1 + p2) = logsumexp(log(p1), log(p2))
#         aggregated_logit = torch.logsumexp(letter_logits, dim=-1)  # [B]
#         logits4_list.append(aggregated_logit)
...
#     # Now compute probabilities and entropy on aggregated logits
#     probs4 = torch.softmax(logits4, dim=-1)
#     entropy = -(probs4 * torch.log(probs4 + 1e-12)).sum(dim=-1)
...
#     # After getting letter_token_ids
#     debug_print = True
#     if debug_print:  # Set to True for one-time debug
#         print("\nDEBUG: Token IDs for ABCD:")
#         for letter in "ABCD":
#             token_ids = letter_token_ids[letter]
#             print(f"  {letter}: {token_ids} -> {[tokenizer.decode([tid]) for tid in token_ids]}")
        
#         # Check actual logits
#         print("\nDEBUG: Sample logits distribution:")
#         print(f"  Logits for ABCD: {logits4[0].tolist()}")
#         print(f"  Probs for ABCD: {probs4[0].tolist()}")
...
# def build_multiple_choice_question_prompts(batch, tokenizer):
#     """Build MCQ prompts matching mentor's OpenRouter format."""
...
#         # Combine setup + question (what gets sent to API)
#         user_content = setup_prompt + "\n\n" + llm_prompt
        
#         # Apply chat template (like OpenRouter does)
#         messages = [{"role": "user", "content": user_content}]
#         prompt = tokenizer.apply_chat_template(
#             messages, 
#             tokenize=False, 
...
# def build_multiple_choice_question_prompts(batch, tokenizer):
#     """Build MCQ prompts with chat template, carefully formatted."""
...
# def build_multiple_choice_question_prompts(batch, tokenizer):
#     """Build MCQ prompts - manually formatted to avoid default system message."""
...
#         # Manually build Llama 3.1 format WITHOUT default system message
#         prompt = (
#             "<|begin_of_text|>"
#             "<|start_header_id|>user<|end_header_id|>\n\n"
#             f"{user_content}<|eot_id|>"
